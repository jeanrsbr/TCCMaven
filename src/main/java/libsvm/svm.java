...
class Cache {
	...
	int get_data(int index, float[][] data, int len)
	{
		...
		if(more > 0)
		{
			...
			h.data = new_data;
			size -= more;
			do {int aux=h.len; h.len=len; len=aux;} while(false);
		}
	...
	void swap_index(int i, int j)
	{
		...
		do {float[] aux=head[i].data; head[i].data=head[j].data; head[j].data=aux;} while(false);
		do {int aux=head[i].len; head[i].len=head[j].len; head[j].len=aux;} while(false);
		if(head[i].len > 0) lru_insert(head[i]);
		if(head[j].len > 0) lru_insert(head[j]);

		if(i>j) do {int aux=i; i=j; j=aux;} while(false);
		for(head_t h = lru_head.next; h!=lru_head; h=h.next)
		{
			if(h.len > i)
			{
				if(h.len > j)
					do {float aux=h.data[i]; h.data[i]=h.data[j]; h.data[j]=aux;} while(false);
				else
				{
					// give up
					lru_delete(h);
...
class Solver {
	...
	{
		Q.swap_index(i,j);
		do {byte aux=y[i]; y[i]=y[j]; y[j]=aux;} while(false);
		do {double aux=G[i]; G[i]=G[j]; G[j]=aux;} while(false);
		do {byte aux=alpha_status[i]; alpha_status[i]=alpha_status[j]; alpha_status[j]=aux;} while(false);
		do {double aux=alpha[i]; alpha[i]=alpha[j]; alpha[j]=aux;} while(false);
		do {double aux=p[i]; p[i]=p[j]; p[j]=aux;} while(false);
		do {int aux=active_set[i]; active_set[i]=active_set[j]; active_set[j]=aux;} while(false);
		do {double aux=G_bar[i]; G_bar[i]=G_bar[j]; G_bar[j]=aux;} while(false);
	}
...
class SVC_Q extends Kernel
{
	...
	void swap_index(int i, int j)
	{
		...
		do {byte aux=y[i]; y[i]=y[j]; y[j]=aux;} while(false);
		do {double aux=QD[i]; QD[i]=QD[j]; QD[j]=aux;} while(false);
	}
}
...
class SVR_Q extends Kernel
{
	...
	void swap_index(int i, int j)
	{
		do {byte aux=sign[i]; sign[i]=sign[j]; sign[j]=aux;} while(false);
		do {int aux=index[i]; index[i]=index[j]; index[j]=aux;} while(false);
		do {double aux=QD[i]; QD[i]=QD[j]; QD[j]=aux;} while(false);
	}
...
public class svm {
	...
	private static void svm_binary_svc_probability(svm_problem prob, svm_parameter param, double Cp, double Cn, double[] probAB)
	{
		...
		for(i=0;i<prob.l;i++)
		{
			...
			do {int aux=perm[i]; perm[i]=perm[j]; perm[j]=aux;} while(false);
		}
		for(i=0;i<nr_fold;i++)
		{
	...
	private static void svm_group_classes(svm_problem prob, int[] nr_class_ret, int[][] label_ret, int[][] start_ret, int[][] count_ret, int[] perm)
	{
		...
		{
			do {int aux=label[0]; label[0]=label[1]; label[1]=aux;} while(false);
			do {int aux=count[0]; count[0]=count[1]; count[1]=aux;} while(false);
			for(i=0;i<l;i++)
			{
				if(data_label[i] == 0)
					data_label[i] = 1;
		    ...
		    param.svm_type == svm_parameter.NU_SVC) && nr_fold < l)
		{
			...
			for (c=0; c<nr_class; c++)
				for(i=0;i<count[c];i++)
				{
					...
					do {int aux=index[start[c]+j]; index[start[c]+j]=index[start[c]+i]; index[start[c]+i]=aux;} while(false);
				}
			for(i=0;i<nr_fold;i++)
			{
				fold_count[i] = 0;
